...
SetOption132<a class="cmnd" id="setoption132"></a>|When MQTT TLS or telegram is enabled, forces fingerprint validation of server identity instead of checking the identify against a certificate authority (CA)<BR>`1` = Fingerprint, `0` = CA. Note that the telegram api will fail to connect if this option is not set.
